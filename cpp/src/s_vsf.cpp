...
{
    limited_list<S2M_Snp_type> s2m_snp_empty;
    ...
    if (rd_data.valid)
    {
        ...
        {
            CPP_WARNING("rd_data Valid but not found in vsf_line");
        }
    }
    ...
    if (vsf_line.size() > vsf_line_size)
        CPP_WARNING("ERROR: vsf_line size exceed limit");
    ...
    const S2M_Snp_type &s2m_snp_provide_bitag)
{
    ...
    if (m2s_rsp.valid)
    {
        ...
        {
            CPP_WARNING("ERROR: BITag not found in mapper");
        }
    }
    ...
    if (m2s_req_dpa.valid)
    {
        if (decoder.is_host_pid(m2s_req_dpa.spid))
            ...
            CPP_WARNING("ERROR: m2s_req_dpa.spid is not host pid");
    }
    if( m2s_rwd_dpa.valid && !decoder.is_host_pid(m2s_rwd_dpa.spid))
        CPP_WARNING("ERROR: m2s_rwd_dpa.spid is not host pid");

    if (m2s_req_list.size() > m2s_req_list_size)
        CPP_WARNING("ERROR: m2s_req_list size > m2s_req_list_size");

    limited_list<S2M_Snp_type> s2m_snp_dpa;
    limited_list<S2M_NDR_Type> s2m_ndr_dpa;
    limited_list<S2M_DRS_Type> s2m_drs_dpa;
    ...
    if (s2m_snp_not_alloc_bitag.size() > s2m_snp_not_alloc_bitag_size)
        CPP_WARNING("ERROR: s2m_snp_not_alloc_bitag size > s2m_snp_not_alloc_bitag_size");
#endif
    if (mapper.size() > mapper_size)
        CPP_WARNING("ERROR: mapper size > mapper_size");
}