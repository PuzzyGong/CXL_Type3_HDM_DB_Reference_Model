    ...
    const M2S_RwD_Type &m2s_rwd)
{
    ...
    limited_list<S2M_Snp_type> s2m_snp_tmp;
    limited_list<S2M_NDR_Type> s2m_ndr_tmp;
    limited_list<S2M_DRS_Type> s2m_drs_tmp;
    limited_list<Rd_Addr_Type> rd_addr_tmp;
    limited_list<Wr_Type> wr_tmp;
    limited_list<CXL_io_Rsp_Type> cxl_io_rsp_tmp;
...
void S_Test_Type::process_chunk(5port，const std::string &chunk, char end_char)
{
    // 正则匹配非英文字符作为分隔符
