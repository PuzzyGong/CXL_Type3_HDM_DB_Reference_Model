    ...
    std::list<Wr_Type> &wr)
{
    ...
    for (auto it_list = m2s_req_list.begin(); it_list != m2s_req_list.end();)
    {
        ...
        int tag = m2s_req_single.address;
        auto it = vsf_line.find(tag);
        if (it != vsf_line.end())
        {
            if (!it->second.isIdle())
    ...
    const Victim_Type &victim)
{
    ...
    decoder.decode(m2s_rsp, m2s_rsp_dpa); // m2s_rsp_dpa <- m2s_rsp
    ...
    std::list<S2M_Snp_type> s2m_snp_hpa;
    std::list<S2M_NDR_Type> s2m_ndr_hpa;
    std::list<S2M_DRS_Type> s2m_drs_hpa;
    main_product(m2s_rsp_dpa, m2s_rwd_dpa, s2m_snp_hpa, s2m_ndr_hpa, s2m_drs_hpa, rd_data, rd_addr, wr);
